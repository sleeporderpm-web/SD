...
def classify(inputs: Dict) -> Tuple[str, int]:
    """
    Classify sleep disorder based on input parameters using ML rules.
    
    Returns:
        Tuple of (diagnosis, severity_level)
        Severity levels: 0=Normal, 1=Review, 2=Moderate, 3=High Risk
    """
    ...
    # Calculate Obstructive Sleep Apnea (OSA) risk score
    osa_score = 0
    ...
    # Calculate Sleep Deprivation risk score
    deprivation_score = 0
    if duration < 6:
        deprivation_score += 2
    if hours_work >= 9:
    ...
    # Classification logic using ensemble approach
    if osa_score >= 5:
